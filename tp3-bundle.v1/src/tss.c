...
void tss_inicializar() {
	...
	tss_idle.unused4 = 0;
	tss_idle.cs = ( GDT_IDX_KERN_CODE << 3 );
	tss_idle.unused5 = 0;
	tss_idle.ss = ( GDT_IDX_KERN_DATA << 3 );
	tss_idle.unused6 = 0;
	tss_idle.ds = ( GDT_IDX_KERN_DATA << 3 );
	tss_idle.unused7 = 0;
	tss_idle.fs = ( GDT_IDX_KERN_DATA << 3 );
	tss_idle.unused8 = 0;
	tss_idle.gs = ( GDT_IDX_KERN_DATA << 3 );
	tss_idle.unused9 = 0;
	tss_idle.ldt = 0;
	tss_idle.unused10 = 0;
	tss_idle.dtrap = 0;
...
tss* tss_crear_tarea(taskType tipo, unsigned short id_tarea, int gdt_index, unsigned int fisica){
	...
	new_tss->esi = 0;
	new_tss->edi = 0;
	new_tss->es = ( GDT_IDX_USER_DATA << 3 ) + 3;
	new_tss->unused4 = 0;
	new_tss->cs = ( GDT_IDX_USER_CODE << 3 ) + 3;
	new_tss->unused5 = 0;
	new_tss->ss = ( GDT_IDX_USER_DATA << 3 ) + 3;
	new_tss->unused6 = 0;
	new_tss->ds = ( GDT_IDX_USER_DATA << 3 ) + 3;
	new_tss->unused7 = 0;
	new_tss->fs = ( GDT_IDX_USER_DATA << 3 ) + 3;
	new_tss->unused8 = 0;
	new_tss->gs = ( GDT_IDX_USER_DATA << 3 ) + 3;
	new_tss->unused9 = 0;
	new_tss->ldt = 0;
	new_tss->unused10 = 0;
	new_tss->dtrap = 0;
